...
delegate a bulk-read or bulk-draft step to `adlc-read` (or its legacy `ask-kimi` shim) or fall back to
...
- **0 — delegated**: `adlc-read` is on PATH AND `ADLC_DISABLE_KIMI` is not `1`. Run the delegated path.
...
case $gate in
  0) # delegated path — invoke adlc-read, capture stdout, post-validate
     ;;
  1) # disabled path — emit "/<skill>: adlc-read disabled via ADLC_DISABLE_KIMI — <purpose>"
     ;;
...
- Disabled (return 1):    `/<skill>: adlc-read disabled via ADLC_DISABLE_KIMI — <purpose>`
...
```sh
grep -l 'command -v adlc-read.*ADLC_DISABLE_KIMI' */SKILL.md
```

must remain empty across the toolkit. Any skill containing `ADLC_DISABLE_KIMI`
(or the new `ADLC_DISABLE_DELEGATE`) MUST also source a gate partial
(`partials/kimi-gate.sh` or `partials/delegate-gate.sh`). `lint-skills`
enforces this with both anchors (REQ-515 BR-4 / ADR-9).
