...
All deployment execution files (implementation plan, task tracker, state status, and log output) must be collected under a unique run directory: `.adlc/deployment/<run-id>/`, where `<run-id>` is generated in the `YYYYMMDD-HHMMSS` format (e.g. `20260521-155711`).
...
4. Generate a unique Run ID using the format `YYYYMMDD-HHMMSS` (e.g. `20260521-155711`) and create the run directory at `.adlc/deployment/<run-id>/`.
5. Copy or save the active `implementation_plan.md` and `task.md` for this deployment run inside `.adlc/deployment/<run-id>/`.
6. Log status: "Preflight verified. Initializing deployment pipeline for run <run-id>."
7. Initialize `.adlc/deployment/<run-id>/deploy-status.json` with Phase 0 status: `success`, and set the overall status to `in-progress`.
...
4. Update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 1 status: `success` and detected tech stack details.
...
5. Update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 2 status: `success` and list of configured variables (redacting secret values).
...
5. Update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 3 status: `success` and target provisioning details.
...
4. Save the full build/startup output to `.adlc/deployment/<run-id>/deploy.log`.
5. If deployment succeeds (health check HTTP 200 OK), update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 4 status: `success` and skip to Phase 6.
6. If deployment fails (build crash, Nginx 502, timeout), ensure logs are fully captured in `.adlc/deployment/<run-id>/deploy.log`, update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 4 status: `failed`, and proceed to Phase 5.

---

### Phase 5: Self-Healing & Redeployment Loop
**Action:** Invoke `/deploy-heal` with the path to the error log file `.adlc/deployment/<run-id>/deploy.log`.
...
5. **Iteration Limit:** Allow up to **3 healing iterations**. If the deployment still fails after 3 tries, update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 5 status: `failed` (overall status: `failed`), halt and present the parsed logs and recommended fixes to the user.
6. If the redeploy loop succeeds, update `.adlc/deployment/<run-id>/deploy-status.json` with Phase 5 status: `success` and applied fixes details.
