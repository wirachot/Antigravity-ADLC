...
def test_ask_kimi_notice_fires_by_default(tmp_path):
    ...
    r = _run([ASK_KIMI, "--paths", str(src), "--question", "q"], env=_env_without_key())
...
def test_ask_kimi_notice_suppressed_by_flag(tmp_path):
    src = tmp_path / "x.txt"
    src.write_text("hello\n")
    r = _run([ASK_KIMI, "--paths", str(src), "--question", "q", "--no-warn"], env=_env_without_key())
...
def test_ask_kimi_notice_suppressed_by_env(tmp_path):
    src = tmp_path / "x.txt"
    src.write_text("hello\n")
    r = _run([ASK_KIMI, "--paths", str(src), "--question", "q"], env=_env_without_key(KIMI_NO_WARN="1"))
    assert _NOTICE_SUBSTR not in r.stderr
    assert r.returncode != 0
...
def test_ask_kimi_dry_run_does_not_emit_notice(tmp_path):
    src = tmp_path / "x.txt"
    src.write_text("hello\n")
    r = _run([ASK_KIMI, "--paths", str(src), "--question", "q", "--dry-run"], env=_env_without_key())
    assert _NOTICE_SUBSTR not in r.stderr
...
def test_kimi_write_notice_fires_by_default(tmp_path):
    ...
    r = _run([KIMI_WRITE, "--spec", "anything", "--target", str(target)], env=_env_without_key())
    assert _NOTICE_SUBSTR in r.stderr
    assert r.returncode != 0
...
def test_kimi_write_notice_suppressed_by_flag(tmp_path):
    target = tmp_path / "out.py"
    r = _run([KIMI_WRITE, "--spec", "anything", "--target", str(target), "--no-warn"], env=_env_without_key())
    assert _NOTICE_SUBSTR not in r.stderr
    assert r.returncode != 0
...
def test_kimi_write_notice_suppressed_by_env(tmp_path):
    target = tmp_path / "out.py"
    r = _run([KIMI_WRITE, "--spec", "anything", "--target", str(target)], env=_env_without_key(KIMI_NO_WARN="1"))
    assert _NOTICE_SUBSTR not in r.stderr
    assert r.returncode != 0
